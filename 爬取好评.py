...
#生成器函数
def generateUrl(url):
    for j in range(19,len(province_list)):
        ...
        global comment_count_file_path
        comment_count_file_path=comment_count_base_path+comment_count_base_name+keyword+'.csv'
        #创建并初始化两个文件
        init()
...
def isRepeat(jsonText):
    ...
    if comment_list==[] or tag_list==[]:
        ...
        #没commentList字段，但是有好评
        if result[1] != '0':
            print("reason-->" + "未得到comment_list字段，但是有好评")
            return True
    else:#tag_list有内容且（comment_list有内容 或者 没有commentList属性且的确好评为0）
        return False
...
def processDataAndRetrunList(jsonText):
    ...
    comment_list = data_dict['data'].get('commentList','')
    #commentCount
    comment_sum_count = data_dict['data'].get('commentCount',0)
    results=[]
    results.append(processTagList(tag_list,str(comment_sum_count)))
    #没有评论
    if comment_list=='':
        results.append([current_sight_id,' ',' ','1'])
        return results
    txt=''
    ...
    results.append([current_sight_id,txt_n,txt_a,'1'])
    return results
...
def init():
    if os.path.exists(filePath)==False:
        ...
        file.close()
    if os.path.exists(comment_count_file_path) == False:
        file = open(comment_count_file_path, 'w', encoding='utf-8-sig')
        file.write(",".join(attr_list_2) + '\n')
        file.close()
...
def initDirectory():
    if os.path.exists(basePath)==False:
        os.mkdir(basePath)
    if os.path.exists(comment_count_base_path)==False:
        os.mkdir(comment_count_base_path)
...
def main():
    ...
    for url in generateUrl(url_interface):
        ...
        writeCommentCountToFile(results[0])
        writeDataToFile(results[1])
...
#评论数文件的头部
attr_list_2=['sightId','goodCommentAmount','badCommentAmount','sumAmount']
basePath='./goodComment'
baseName='/goodComment_'
filePath=''
comment_count_base_path='./commentCount'
comment_count_base_name='/commentAmount_'
comment_count_file_path=''
...
url_interface='https://piao.qunar.com/ticket/detailLight/sightCommentList.json?index=1&page=1&pageSize=500&tagType=1&sightId='
